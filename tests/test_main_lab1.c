...
void main()
{
  ...
  for ( i = 1 ; i <= create_count_from_main_MAX ; i++ )
  {
      ...
      { 
        ...
        if ( (unsigned int) nextListObj->pTask->PC   != (unsigned int) (  *lowEndPtr )   ) { g1 = FAIL; break;} 
        if ( (unsigned int) nextListObj->pTask->SPSR != (unsigned int) ( *highEndPtr )   ) { g1 = FAIL; break;} 
        if ( (unsigned int) nextListObj->pTask->PC == (unsigned int) 0 )                   { g1 = FAIL; break;}
        // is the newly created task here ?
        if ( (unsigned int) nextListObj->pTask->Deadline != (unsigned int) ( low_deadline + i ) ) { g1 = FAIL; break;}
